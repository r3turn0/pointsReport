function main(workbook: ExcelScript.Workbook) {
    ...
    // const sourceSheet = workSheets.foreach(s =>
    //     s.getName().toLowerCase().includes(sourceSheetName.toLowerCase())
    // );
    ...
    // const matrixSheet = workSheets.foreach(s =>
    //     s.getName().toLowerCase().includes(matrixSheetName.toLowerCase())
    // );
    for(var i=0; i<workSheets.length; i++){
    ...
    if (matrixSheet) {
        ...
        for (let i = 1; i < matrixData.length; i++) {
            const workerName = String(matrixData[i][nameIdx]);
